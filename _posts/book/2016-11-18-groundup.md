...
The malloc code in the example always iterates through all memory segment, which means it runs in **linear time**. Better would be an algorythm that runs in **constant time**.
...
increasing size with  **sign extension**

## Octal and Hexadecimal Numbers

ocatl grouping of three bits, linux permissions

hexadecimal, or base 16
one digit represents 4 bits which is half a byte
so two digits represent one byte


## Order of Bytes in a Word

The x86 processor is a **little-endian** processor which meanst that it stores the "little end", or least significant byte first in memory.
